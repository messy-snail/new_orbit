...
ascending_color = 'blue'  # 상승 궤도 색상
descending_color = 'red'  # 하강 궤도 색상

for i, (satellite, name) in enumerate(tqdm(satellites)):
    lats, lons = [], []
    line_color = ascending_color  # 초기 색상을 상승 궤도 색상으로 설정
    ...
    while start_date.tt < end_date.tt:
        ...
        if prev_lat is not None:
            if prev_lat < 0 and lat >= 0:
                # 하강 궤도에서 상승 궤도로 전환
                m.plot(lons, lats, color=line_color, linestyle=linestyle, linewidth=3)  # 현재 색상의 선 그리기
                lats, lons = [], []
                line_color = ascending_color  # 상승 궤도 색상으로 변경
            elif prev_lat >= 0 and lat < 0:
                # 상승 궤도에서 하강 궤도로 전환
                m.plot(lons, lats, color=line_color, linestyle=linestyle, linewidth=3)
                lats, lons = [], []
                line_color = descending_color  # 하강 궤도 색상으로 변경
    ...
    line, = m.plot(lons, lats, color=line_color, linestyle=linestyle, linewidth=3)
